...
if use_wandb:
    ...
    'parameters': {
        ...
        'learning_rate': {
            'values': [7e-5, 6e-5,  4e-5, 3e-5, 2e-5]
        },
        'crf_learning_rate': {
            'values': [7e-5, 6e-5, 4e-5, 3e-5, 2e-5]
        },
...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.model_name_or_path in ["gpt2", 'gpt2-large']:
        gpt2_param_optimizer = list(model.gpt2.named_parameters())
    else:
        bert_param_optimizer = list(model.bert.named_parameters())
    crf_param_optimizer = list(model.crf.named_parameters())
    #crf_param_optimizer = list(model.lstmcrf.named_parameters())
    linear_param_optimizer = list(model.classifier.named_parameters())
    optimizer_grouped_parameters = [

        {'params': [p for n, p in gpt2_param_optimizer if not any(nd in n for nd in no_decay)],
         'weight_decay': args.weight_decay, 'lr': args.learning_rate},
        {'params': [p for n, p in gpt2_param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                optimizer.step()
                scheduler.step()  # Update learning rate schedule scheduler.step()should be called after optimizer.step()
                model.zero_grad()
                global_step += 1
