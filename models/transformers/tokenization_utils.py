...
class PreTrainedTokenizer(object):
    ...
    Class attributes (overridden by derived classes):
        ...
        - ``vocab_files_names``: a python ``dict`` with, as keys, the ``__init__`` keyword name of each vocabulary
            file required by the model, and as associated values, the filename for saving the associated file (string).
        - ``pretrained_vocab_files_map``: a python ``dict of dict`` the high-level keys being the ``__init__``
            keyword name of each vocabulary file required by the model, the low-level being the `short-cut-names` (string)
            of the pretrained models with, as associated values, the `url` (string) to the associated pretrained vocabulary
            file.
        - ``max_model_input_sizes``: a python ``dict`` with, as keys, the `short-cut-names` (string) of the pretrained
            models, and as associated values, the maximum length of the sequence inputs of this model, or None if the model
            has no maximum input size.
        - ``pretrained_init_configuration``: a python ``dict`` with, as keys, the `short-cut-names` (string) of
           the pretrained models, and as associated values, a dictionnary of specific arguments to pass to
           the ``__init__``method of the tokenizer class for this pretrained model when loading the tokenizer
    ...
    def _from_pretrained(cls, pretrained_model_name_or_path, *init_inputs, **kwargs):
        ...
        init_kwargs.update(kwargs)#dict
        ...
        #采用gpt2 tokenizer需要用420-427行
        # if pretrained_model_name_or_path in ["gpt2",'gpt2-large']:
        #     init_kwargs2 = {'do_lower_case': False, 'vocab_file': init_kwargs['vocab_file'], 'merges_file': init_kwargs['merges_file']}
        #     tokenizer = cls(**init_kwargs2)
        #     #为啥需要把*init_inputs去掉？ 俺不知道
        # else:
        #     init_kwargs2 = {'do_lower_case': False, 'vocab_file': init_kwargs['vocab_file'] }
        #     #为啥需要把init_kwargs 里的max_len去掉？ 俺不知道
        #     tokenizer = cls(*init_inputs, **init_kwargs2)

        #gpt2采用bert tokenizer用以下两行
        init_kwargs2 = {'do_lower_case': False, 'vocab_file': init_kwargs['vocab_file']}
        tokenizer = cls(**init_kwargs2)
