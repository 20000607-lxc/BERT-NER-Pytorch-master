...
class GPT2SoftmaxForNer_filling_entity(torch.nn.Module):
    ...
    def __init__(self, config, device, template, model_name=None):
        ...
        self.gpt2 = self.LMgpt2.base_model
        self.embeddings = self.gpt2.get_input_embeddings().to(device)
    ...
    def get_query(self, input_id, prompt_tokens, removed_input_ids):
        ...
        for i in range(len(input_id)):
            if input_id[i] != 0:
                ...
                removed.append(removed_input_ids[i].item())

        query = prompt1 + input + prompt2 + removed
        return query, count
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        queries = []
        for i in range(bz):
            # todo note : for test  removed_input_ids == input_ids
